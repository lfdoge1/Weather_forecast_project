...
raw_data_path = os.path.join(data_dir, "coventry_data.csv")
df = pd.read_csv(raw_data_path, parse_dates=["datetime"], index_col="datetime")
...
clean_data_path = os.path.join(output_dir, "weather_coventry_clean.csv")
...
# Save transform
joblib.dump(dew_scaler, os.path.join(output_dir, "dew_scaler_coventry.pkl"))
joblib.dump(humidity_scaler, os.path.join(output_dir, "humidity_scaler_coventry.pkl"))
joblib.dump(wind_scaler, os.path.join(output_dir, "wind_scaler_coventry.pkl"))
joblib.dump(month_scaler, os.path.join(output_dir, "month_scaler_coventry.pkl"))
joblib.dump(year_scaler, os.path.join(output_dir, "year_scaler_coventry.pkl"))
joblib.dump(target_transformer, os.path.join(output_dir, "temperature_scaler_coventry.pkl"))
...
r2_rnn = r2   # save for later compare
...
#
# # Plot RMSE comparison graph of different units
...
lstm_model = Sequential()
lstm_model.add(LSTM(64, activation='tanh', input_shape=(sequence_length, X_train.shape[2])))
lstm_model.add(Dense(1))
lstm_model.compile(optimizer='adam', loss='mse')
...
# To packet
model_path = os.path.join(output_dir, "lstm_temperature_model_coventry.h5")
...
model = Sequential()
model.add(Bidirectional(LSTM(32, activation='tanh', input_shape=(sequence_length, X_train.shape[2]))))#Lstm original=100
model.add(Dense(1))
...
history = model.fit(X_train, y_train, epochs=30, validation_data=(X_test, y_test), batch_size=16,
                    callbacks=[early_stopping]) #original batch_size=16
...
r2_bilstm = r2 #later for compare
...
# r2_scores = [r2_rnn, r2_lstm, r2_bilstm]
#
